...
/*
	Grab the high and low dimensional distances for a point
	Compute the squared differences between them and
...
void main( ) {
	...
	while( i < 2 ) {
		...
		high_d.x = (high_d.x==1000.0)?low_d.x:high_d.x;
		high_d.y = (high_d.y==1000.0)?low_d.y:high_d.y;
		high_d.z = (high_d.z==1000.0)?low_d.z:high_d.z;
		high_d.w = (high_d.w==1000.0)?low_d.w:high_d.w;

		// compute the squared differences and sum	

		diff 	= high_d - low_d;
		stress 	= stress + dot(diff,diff);
	...
	gl_FragColor = vec4(stress,0.0,0.0,0.0);
