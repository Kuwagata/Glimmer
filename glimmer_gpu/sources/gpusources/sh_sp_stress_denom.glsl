...
void main( ) {
	...
	float stress_high 	= 0.0;	// final denominator high^2
	...
	while( i < 2 ) {
		...
		high_d 	= texture2DRect(t_d,self);
		low_d 	= texture2DRect(t_g,self);
		...
		stress_high	+= dot(high_d,high_d);
		stress_low 	+= dot(low_d,low_d);

		//stress 	+= ((choose_low>1.0)?dot(low_d,low_d):dot(high_d,high_d));
	...
	gl_FragColor = vec4(((choose_low>1.0)?stress_low:stress_high),0.0,0.0,0.0);//vec4(stress,0.0,0.0,0.0);
